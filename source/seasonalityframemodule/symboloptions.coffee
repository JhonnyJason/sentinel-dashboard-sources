...
import { provideSearchOptions } from "./seasonalityframemodule.js"
...
############################################################
export dynamicSearch = (searchString) ->
    log "dynamicSearch"
    if blocked
    ...
    blocked = true
    await retrieveOptions(searchString)
    setTimeout(unblock, cooldownMS)
    return

############################################################
retrieveOptions = (searchString) ->
    log "retrieveOptions"
    options = []
    try options = await getSymbolOptions(searchString)
    catch err then log err
    provideSearchOptions(options)
    return
